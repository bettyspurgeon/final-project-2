...
class MatchhomeController extends Controller
{
    ...
    public function display_matches($id)
    {
        ...
        foreach ($landlord_preferences as $landlord_preference) {
            if ($landlord_preference['income'] <= $profile->income) {
                if (
                    ($landlord_preference['contract'] === 'NONE') ||
                    ($landlord_preference['contract'] === $profile->contract) ||
                    ($landlord_preference['contract'] === 'CDD' && ($profile[0]->contract === 'CDD' || $profile->contract === 'CDI'))
                ) {
